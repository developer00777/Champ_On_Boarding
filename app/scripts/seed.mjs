...
// Plain ESM — no TypeScript, runs with: node scripts/seed.mjs
//
...
// doubly harmful: this script runs on every Railway deploy via start.sh, and
// its upsert force-set `active: true` — silently resurrecting every company an
// admin had soft-deleted from /admin/entities — and it matched on outdated
// short names ("Champion Infratech"), inserting duplicate rows next to the
// canonical "… Pvt Ltd" ones on each fresh deploy.
...
if (!existing) {
	...
	console.log(`[seed] super admin already exists, skipping: ${SUPER_ADMIN_EMAIL}`);
}
